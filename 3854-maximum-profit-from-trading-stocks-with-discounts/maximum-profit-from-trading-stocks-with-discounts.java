class solve {
    ...
    solve(int[] dp0, int[] dp1, int size) {
        this.dp0 = dp0;
        this.dp1 = dp1;
        this.size = size;
    }
...
class Solution {
    ...
    private solve dfs(
        ...
        for (int v : g[u]) {
            solve childsolve = dfs(v, present, future, g, budget);
            uSize += childsolve.size;

            for (int i = budget; i >= 0; i--) {
                for (int sub = 0; sub <= Math.min(childsolve.size, i); sub++) {
                    if (i - sub >= 0) {
                        subProfit0[i] = Math.max(
                            subProfit0[i],
                            subProfit0[i - sub] + childsolve.dp0[sub]
                        );
                        subProfit1[i] = Math.max(
                            subProfit1[i],
                            subProfit1[i - sub] + childsolve.dp1[sub]
                        );
                    }
                }
            }
        ...
        return new solve(dp0, dp1, uSize);
    }
}